...
type Options struct {
	World       string // world directory containing the player data dir
	Manifest    string // trace manifest; positions come from the traces it lists
	Prefix      string // username prefix, must match identity.username_prefix
	Count       int    // number of accounts, must match load.target_players
	Origin      string // fallback "x,y,z" for traces carrying none
	GameMode    int
	DataVersion int    // 0 = read from the world's level.dat
	Dir         string // explicit player data dir; empty = auto-detect
	ImportDir   string // real players' .dat files to hand to the bots
	PlayerMap   string // "hash uuid" lines, for anonymised captures
	SaltHex     string
	Remove      bool
	DryRun      bool
}
...
func Defaults() Options { return Options{Prefix: "BENCH_"} }

// Run writes (or removes) the bench accounts' player data.
func Run(o Options) error {
	world, manifest, prefix := &o.World, &o.Manifest, &o.Prefix
	count, originFlag, gameMode := &o.Count, &o.Origin, &o.GameMode
	dataVersion, dirFlag, importDir := &o.DataVersion, &o.Dir, &o.ImportDir
	playerMap, saltHex := &o.PlayerMap, &o.SaltHex
	remove, dryRun := &o.Remove, &o.DryRun

	if *world == "" || *count <= 0 {
		return fmt.Errorf("playerdata: --world and a positive --count are required")
	}
	...
	if n := len(*prefix); n > 11 {
		return fmt.Errorf("--prefix %q is %d characters; the 5-digit account index leaves "+
	...
	if *importDir != "" {
		if *manifest == "" {
			return fmt.Errorf("--import needs --manifest: without it there is no way to tell which trace belongs to which player")
		}
		salt := make([]byte, 16)
		if *saltHex != "" {
	...
	if len(origins) == 0 && fallback == nil {
		return fmt.Errorf("no positions available: pass --manifest (with traces that carry an origin) or --origin x,y,z")
	}
	...
	if err := os.MkdirAll(playerDir, 0o755); err != nil {
		return fmt.Errorf("create %s: %w", playerDir, err)
	}
	...
	for i := 0; i < *count; i++ {
		...
		if len(imports) > 0 && imports[i%len(imports)] != nil {
			...
			if err != nil {
				return fmt.Errorf("import for %s: %w", name, err)
			}
			imported++
		} else {
